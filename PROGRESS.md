...
**`electron-builder` has never been executed.** `npm run dist` is configured —
`asar: false`, `includeSubNodeModules: true`, the `files` list generated by the
`nextjs-to-electron` skill's `runtime-deps.js` rather than by hand, and
`signAndEditExecutable: false` so `signtool` cannot hang the build probing an
empty certificate store — but none of it has produced an installer.
...
1. `npm run dist`, then look inside
   `dist/win-unpacked/resources/app/.next/standalone/node_modules/` and confirm
   `next` and `react` are really there. A successful build is not evidence —
   electron-builder deletes that directory from builds that succeed.
